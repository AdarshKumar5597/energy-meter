...
//    val values = values.map { it - minimum }
    val values = values.map { if (java.lang.Float.isNaN(it)) 0f else it }
    val isDataSafe = values.distinct().count() == 1
    val maxOfValues = if (isDataSafe) 2 * values[0] else values.reduce(::max) * 1.1f
    val fractions = values.map { 1 - it / maxOfValues }
    ...
    Canvas(modifier = modifier.size(300.dp)) {
        ...
        (0..levels).forEach {
            ...
            drawContext.canvas.nativeCanvas.drawText(
                (minimum + delta * (levels - it)).toString(),
